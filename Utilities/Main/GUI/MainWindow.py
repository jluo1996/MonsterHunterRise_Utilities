...
from Main.GUI.CustomStatusBar import CustomStatusBar
...
class MainWindow(QMainWindow):
    def __init__(self, main_vm : MainViewModel, logger, parent=None):
        ...
        self.logger.log("Initializing MainViewModel and loading mods...", level="INFO")
        self.main_vm.on_startup() 
        self.main_gui = ModInstallUI(self.main_vm, self.logger)
        self.setCentralWidget(self.main_gui)
        self.logger.log("MainViewModel initialized and mods loaded.", level="INFO")

        file_helper = FileHelper(self.logger)
        self._build_menubar(file_helper)

        if self.main_vm.has_newer_version():
            self.status_bar.showMessage(f"New version {self.main_vm.get_latest_version_number(only_number=True)} available! Check Help > Check for Updates.")
            self.status_bar.set_message_color("orange")
    ...
    def _build_menubar(self, file_helper: FileHelper):
        ...
        act_check_updates = QAction(check_updates_icon, "Check for &Updates", self)
    ...
    def _check_for_updates(self):
        ...
        if not self.main_vm.has_newer_version():
        ...
        latest_version = self.main_vm.get_latest_version_number()
        ...
        finish_callbacks = [updating_dialog.accept,
                           lambda success: setattr(self, 'prepare_success', success)]
        self.main_vm.start_update_preparation_process(finish_callback=finish_callbacks)  # Close the dialog when preparation is done
        ...
        self.main_vm.start_update_process()
    ...
    def _on_log_message_received(self, msg, level):
        ...
        match level:
            case "ERROR":
                self.status_bar.set_message_color("red")
            case "WARNING":
                self.status_bar.set_message_color("orange")
            case "INFO":
